  ...
  return (
    <div className="w-full bg-[#121618] text-white flex items-end justify-between px-6 md:px-16 lg:px-36 py-5 relative">
      ...
      <div className="hidden md:flex gap-8 text-lg font-medium">
        ...
        <Link to="/reservation" className={isActive("/reservation")}>Make a Reservation</Link>
      ...
      <div className="md:hidden z-50">
        {isOpen ? (
          <X className="w-7 h-7 cursor-pointer text-white" onClick={() => setIsOpen(false)} />
        ) : (
          <Menu className="w-7 h-7 cursor-pointer text-white" onClick={() => setIsOpen(true)} />
        )}
      </div>
      ...
      {isOpen && (
        <div className="absolute top-0 left-0 w-full h-screen bg-[#121618] text-white flex flex-col items-center justify-center gap-8 z-40 text-xl font-semibold transition-all duration-300">
          <Link onClick={() => setIsOpen(false)} to="/" className={isActive("/")}>Home</Link>
          <Link onClick={() => setIsOpen(false)} to="/menu" className={isActive("/menu")}>Menu</Link>
          <Link onClick={() => setIsOpen(false)} to="/reservation" className={isActive("/reservation")}>Make a Reservation</Link>
          <Link onClick={() => setIsOpen(false)} to="/contact" className={isActive("/contact")}>Contact Us</Link>
        </div>
      )}
    </div>
  );
